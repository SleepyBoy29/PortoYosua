const buildPalette = (colorsList) => {
  ...
  for (let i = 0; i < orderedByColor.length; i++) {
    ...
    const rgbColor = `(${orderedByColor[i].r}, ${orderedByColor[i].g}, ${orderedByColor[i].b})`;
    const cmykColor = rgbToCMYK(orderedByColor[i]);
    ...
    // create the div and append it to the document
    const colorElement = document.createElement("div");
    colorElement.style.backgroundColor = hexColor;

    // create text nodes for each color information
    const rgbText = document.createTextNode(`RGB: ${rgbColor}`);
    const hexText = document.createTextNode(`Hex: ${hexColor}`);
    const cmykText = document.createTextNode(`CMYK: ${cmykColor}`);

    // append text nodes to the color element
    colorElement.appendChild(rgbText);
    colorElement.appendChild(document.createElement("br")); // line break
    colorElement.appendChild(hexText);
    colorElement.appendChild(document.createElement("br")); // line break
    colorElement.appendChild(cmykText);
    ...
    if (hslColors[i].h) {
      ...
      // create text node for complementary color information
      const complementaryText = document.createTextNode(`Complementary Hex: ${hexColorComplementary}`);
      complementaryElement.appendChild(complementaryText);
...
const rgbToCMYK = (pixel) => {
  ...
  // Handle special case when black
  if (isNaN(c)) c = 0;
  if (isNaN(m)) m = 0;
  if (isNaN(y)) y = 0;
  if (isNaN(k)) k = 1;
